bl_info = {
    ...
    "description": "v0.3.8: 1000MU设计团队专用AI效率插件。双轨制材质分流与高保真动流优化。",
    "category": "3D View",
}
...
def chinese_to_safe_name(s):
    ...
    result = ('layer_' + fallback) if fallback else 'layer'
    return result
...
class MAP_OT_import_svg(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        temp_svg = os.path.join(os.path.dirname(input_file), "temp_blender_map.svg")
...
class MAP_OT_generate_3d(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.ops.object.select_all(action='SELECT'); bpy.ops.object.mode_set(mode='EDIT'); bpy.ops.mesh.select_all(action='SELECT'); bpy.ops.mesh.normals_make_consistent(inside=False); bpy.ops.object.mode_set(mode='OBJECT')
...
class MAP_OT_build_atlas(bpy.types.Operator):
    ...
    bl_label = "生成色块贴图"
    bl_description = "【双轨制引擎】提取色彩与Alpha通道生成全局图集。极速坍缩UV并自动分流实体与透明材质，将 Draw Call 降至极致"
    BLOCK = 64  
    ...
    def execute(self, context):
        ...
        for obj in mesh_objs:
            ...
            if key not in color_groups: color_groups[key] = {'linear': lin, 'objs': []}
            color_groups[key]['objs'].append(obj)
        ...
        for i, (key, data) in enumerate(entries):
            ...
            sr, sg, sb = linear_to_srgb(data['linear'][0]), linear_to_srgb(data['linear'][1]), linear_to_srgb(data['linear'][2])
            a = data['linear'][3]
            ...
            color_uvs[key] = ((col_idx * self.BLOCK + self.BLOCK * 0.5) / img_w, (row_idx * self.BLOCK + self.BLOCK * 0.5) / img_h)
        ...
        for key, data in entries:
            ...
            target_mat = mat_trans if a < 1.0 else mat_opaque
            
            c0u, c0v = uc - HALF, vc - HALF
            c1u, c1v = uc + HALF, vc - HALF
            c2u, c2v = uc + HALF, vc + HALF
            c3u, c3v = uc - HALF, vc + HALF
            ...
            for obj in data['objs']:
                ...
                # 所有 loop UV 统一指向色块正中心。
                # 坐标微偏 ±0.00005 仅为防止 Mipmap 退化为 0 的工程保险，
                # 不依赖 loop%4 的四边面假设，三角面/N-gon 均安全。
                loop_count = len(me.loops)
                uv_flat = [0.0] * (loop_count * 2)
                for li in range(loop_count):
                    # 交替微偏（±HALF）确保 UV 不完全退化为单点
                ...
                me.materials.clear()
                me.materials.append(target_mat) 

        self.report({'INFO'}, f"⚡ 双轨制图集生成完成 (Opaque + Blend 分离)！")
        return {'FINISHED'}
